...
async fn post_fin(api_state: &ApiContext, upload_info: UploadInfo) -> Result<(), TusError> {
    ...
    .call()
    .await
    .map_err(|_e| TusError::Internal("Couldn't enqueue HandleWebUpload job".to_owned()))?;
...
pub fn upload_protected_router(api_state: &ApiContext) -> Router<ApiContext> {
    ...
    let config = Config {
        hooks: HookConfig {
            // todo: how can i pass the pool & settings to the post_fin?
            pre_finish: Some(Arc::new(|info| {
                let cloned_state = api_state.clone();
